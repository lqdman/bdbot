...
//     case 'awaitingBattleName':
//       handleAwaitingBattleName(msg, chatId, userData);
//       break;
//     case 'awaitingBattleDates':
//       handleAwaitingBattleDates(msg, chatId, userData);
//       break;
//     case 'awaitingBattleYear':
//       handleAwaitingBattleYear(msg, chatId, userData);
//       break;
//     case 'awaitingBattleLocation':
//       await handleAwaitingBattleLocation(msg, chatId, userData);
//       break;
//     case 'awaitingBattleDescription':
//       handleAwaitingBattleDescription(msg, chatId, userData);
//       break;
//     case 'awaitingBattleLink':
//       handleAwaitingBattleLink(msg, chatId, userData);
//       break;
//     case 'awaitingBattleCover':
//       handleAwaitingBattleCover(msg, chatId, userData);
//       break;
//     case 'awaitingPracticeName':
//       handleAwaitingPracticeName(msg, chatId, userData); // Предполагается, что у вас есть функция handleAwaitingPracticeName
//       break;
//     default:
//       break;
//   }
...
//     return;
//   }
//   updateUserState(chatId, 'awaitingBattleCover', { battleLink: msg.text });
//   bot.sendMessage(chatId, messages[userData.language].addBattleCover);
// }

// // Функция для обработки состояния 'awaitingBattleCover'
// function handleAwaitingBattleCover(msg, chatId, userData) {
//   if (msg.photo && msg.photo.length > 0) {
//     const photoFileId = msg.photo[msg.photo.length - 1].file_id;
//     updateUserState(chatId, null, { battleCover: photoFileId });
...
//     bot.sendMessage(chatId, `${userLabels.title}: ${battleNameUpperCase}\n—\n${userLabels.dates}: ${userData.postData.battleDates}\n${userLabels.city}: ${userData.postData.battleLocation}\n•••\n\n${description}${userLabels.link}: ${userData.postData.battleLink}\n\n${messages[userData.language].searchByTag} ${tags}`);
//   } else {
//     bot.sendMessage(chatId, messages[userData.language].invalidCover);
//   }
// }
...
//     [{ text: messages['English'].changeMonthButton, callback_data: 'change_month' }] // Добавляем кнопку "Изменить месяц"
//   ];
//   return { inline_keyboard: keyboard };
// };
...
//   return {
//     inline_keyboard: [
//       [{ text: messages[language].skipButton, callback_data: 'skip' }]
//     ]
//   };
// };
...
// // 23. Обработчик callback-запросов
...
//         break;
//       }
//       const selectedDateForMonth = userData.postData.battleDates; // Переименование переменной
//       const selectedMonthForDates = selectedValue; // Переименование переменной
...
//       bot.editMessageText(messages[userData.language].addBattleYear, { chat_id: chatId, message_id: messageId, reply_markup: createYearKeyboard() });
//       break;
//     case 'awaitingBattleYear':
...
//       const selectedMonthForYear = userData.postData.battleMonth; // Переименование переменной
//       const selectedDateForYear = parseInt(userData.postData.battleDates); // Переименование переменной
...
//       updateUserState(chatId, 'awaitingBattleLocation', { battleYear: selectedValue, battleDates: combinedDateYear }); // 1. Обновление состояния на ожидание локации
//       bot.editMessageText(messages[userData.language].addBattleLocation, { chat_id: chatId, message_id: messageId }); // 2. Отправка сообщения для определения локации
//       break;
//     case 'awaitingBattleDescription':
