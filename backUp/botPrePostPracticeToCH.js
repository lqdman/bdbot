...
//     addPracticeCover: "Please upload the cover image for the practice place:",
//   },
//   Russian: {
...
//     addPracticeCover: "Пожалуйста, загрузите обложку для места тренировки:",
//   }
// };
...
//     awaitingPracticeCover: handleAwaitingPracticeCover // Добавлено
//   };
...
//     return;
//   }
//   updateUserState(chatId, 'awaitingPracticeCover', { practiceLink: msg.text });
//   bot.sendMessage(chatId, messages[userData.language].addPracticeCover);
// }


// // 1. Функция для обработки состояния 'awaitingPracticeCover'
// function handleAwaitingPracticeCover(msg, chatId, userData) {
//   if (msg.photo) {
//     const photo = msg.photo[msg.photo.length - 1].file_id;
//     updateUserState(chatId, 'completed', { practiceCover: photo });
...
// // 2. Отправка финального сообщения для тренировок
// function sendPracticeFinalMessage(chatId, userData) {
//   const userLabels = getLabels(userData.language).practice;
//   const description = userData.postData.practiceDescription ? `${userLabels.description}: ${userData.postData.practiceDescription}\n\n` : '';

//   // Преобразование названия города к формату "camelCase"
//   const cityTag = userData.postData.practiceLocation.split(',')[0].split(' ').map(word => {
//     return word.charAt(0).toUpperCase() + word.slice(1).toLowerCase();
//   }).join('');

//   const tags = `#${userLabels.title}${cityTag}`; // Используем метку из labels

//   // Создание кликабельной ссылки для названия тренировки
//   const practiceNameLink = `<a href="${userData.postData.practiceLink}">${userData.postData.practiceName}</a>`;

//   // Проверка на undefined
//   const practiceLink = userData.postData.practiceLink;
//   if (!practiceLink) {
//     console.error('practiceLink is undefined');
//     bot.sendMessage(chatId, 'Error: practiceLink is undefined');
//     return;
//   }

//   // Преобразование названия города к верхнему регистру для каждого слова
//   const formattedLocation = userData.postData.practiceLocation.split(', ').map(part => {
...
//     [{ text: messages[userData.language].moreInfoButton, url: practiceLink }]
//   ];

//   bot.sendPhoto(chatId, userData.postData.practiceCover, {
//     caption: `${userLabels.title}: ${practiceNameLink}\n\n—\n${userLabels.city}: ${formattedLocation}\n•••\n\n${description}\n\n${messages[userData.language].searchByTag} ${tags}`,
//     reply_markup: {
...
//   }).catch(error => {
//     console.error('Error sending final practice message:', error);
//     bot.sendMessage(chatId, 'Failed to send final practice message. Please check logs for details.');
//   });
// }
...
// // 25. Отправка финального сообщения для баттлов
// function sendFinalMessage(chatId, userData) {
//   const userLabels = getLabels(userData.language).battle;
...
// // 42. Функция для публикации сообщения в канал
...
//     awaitingPracticeDescription: handleAwaitingPracticeDescriptionCallback // Добавлено
//   };
